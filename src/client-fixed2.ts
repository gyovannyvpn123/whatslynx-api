...
export class WhatsLynxClient extends EventEmitter {
  ...
  async disconnect(reason: string = 'client_disconnect'): Promise<void> {
    ...
    try {
      ...
      // Disconnect socket
      // Convert reason from string to number if needed by the socket implementation
      const numericReason = typeof reason === 'string' ? 1000 : reason;
      await this.socket.disconnect(numericReason);
